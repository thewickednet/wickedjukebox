            ...
            {foreach from=$QUEUE item=QUEUE_SONG}
								<li><a href="/details/song/{$QUEUE_SONG.song_id}/">{$QUEUE_SONG.name} - {$QUEUE_SONG.title} ({$QUEUE_SONG.duration|date_format:"%M:%S"}) [{$QUEUE_SONG.fullname}]</a>{if $PERMISSIONS.queue_remove eq '1' || $USERINFO.user_id eq $QUEUE_SONG.queue_user}<a href="#" onclick="javascript:delsong({$QUEUE_SONG.queue_id});" style="color: red;"> del</a>{/if}<br /></li>
            {/foreach}
