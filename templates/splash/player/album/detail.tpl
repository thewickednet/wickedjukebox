            ...
            <h2>{if $CORE->permissions.queue_add eq '1'} <a href="javascript:;" onclick="javascript:addalbum({$ALBUM.id});"><img src="/images/add.png" title="queue the entire album" class="button" /></a> {if $ALBUM.love.count ne '0'}<a href="javascript:;" onclick="javascript:queueAlbumFavourites({$ALBUM.id});"><img src="/images/heart.png" title="Queue all my favourites from this album" border="0" /></a>{/if}&nbsp;{/if} {$ALBUM.name} ({$ALBUM.cost} <img src="/images/money.png" class="button" />)
            ...
            {foreach from=$SONGS item=SONG}
              <tr {if $SONG.standing ne ''}class="{$SONG.standing}"{/if}>
                ...
                <td><img src="/popgraph.php?value={$SONG.prating}" title="{$SONG.num_loves[0].counter|default:'0'} loves, {$SONG.num_hates[0].counter|default:'0'} hates" /></td>
              </tr>
            {/foreach}
            </table>
            </div>
