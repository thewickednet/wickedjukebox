...
def construct_string( user_id ):
   ...
   buffer = StringIO()
   frobj = join( usersTable, songStandingTable, and_(usersTable.c.id == songStandingTable.c.user_id, usersTable.c.id == user_id) )

   main_join = outerjoin( songTable, frobj )

   x = select( [songStandingTable.c.standing+0], from_obj=[main_join], order_by=[songTable.c.id] )
...
if __name__ == "__main__":
   ...
   print "Similarity between user %2d and user %2d: %3.2f%%" % ( user_1, user_2, similarity( construct_string( user_1 ), construct_string( user_2 ) ) * 100 )
