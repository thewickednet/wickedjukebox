        ...
        while self.is_playing:
            try:
                self.position = (
                    ...
                    int(
                        self.player.query_duration(gst.FORMAT_TIME, None)[0]
                        * 1e-9
                    ),
                )
...
def config(params):
    ...
    if "admin_url" in params:
        STATE.admin_url = (
            str(params["admin_url"]) + "/listclients.xsl?mount=" + STATE.mount
        )
...
def getPosition():
    if not STATE.streamer:
        return (0, 100)
    return STATE.streamer.current_position()


def getSong():
    if STATE.streamer:
        return STATE.streamer.filename
    return None
...
def startPlayback():
    ...
    __stream_file(STATE.server, STATE.queue.pop(0))


def status():
    if STATE.streamer:
        return STATE.streamer.status()
    else:
        return "stopped"
...
def current_listeners():
    ...
    if (
        STATE.admin_url is None
        or STATE.admin_username is None
        or STATE.admin_password is None
    ):
        # not all required backend parameters supplied
        LOG.warning(
            "Not all parameters set for screen scraping icecast statistics. Need admin-url and admin-password"
        )
        return
