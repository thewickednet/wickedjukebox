...
def fetch_candidates( channel_id ):
    """
    Fetch a set of candidate songs from the DB
    """
    try:
        # get settings
        ...
        # first, we fetch a limited number of songs using the basic stats. This
        # will prevent too many queries when determining love/hate stats.
        ...
        for row in rough_query.execute():
            ...
            hate_count = _get_standing_count(
                    row[0],
                    users_affecting_hate,
                    'hate')
            if hate_count > 0:
                continue

            # count the loves, for points calculation
            love_count = _get_standing_count(
                    row[0],
                    users_affecting_love,
                    'love')
            ...
            # We then zero-pad this, reverse the string and prefix the score.
            # This will give us a usable sort key to sort by score descending,
            # then by recency ascending.
            delta = datetime.now() - (row[2] and
                    row[2] or
                    datetime( 1900, 01, 01 ))
