...
class Song(object):
    ...
    def scan_from_file(self, localpath, encoding):
        ...
        try:
            ...
            logger.warning(
                "%r contained invalid metadata. Error message: %r" % (localpath, str(ex)))
    ...
    def get_album_id(self, dirname):
        ...
        if not self.artist_id or not self.__albumName:
            logger.warning(
                "Unable to determine the album ID without a valid srtist_id and album-name!")

        artist_id = self.artist_id
        album_name = self.__albumName

        session = create_session()
        album = session.query(Album).selectfirst_by(
...
mapper(Artist, artistTable, properties=dict(
    albums=relation(Album, backref='artist',
                    primaryjoin="album.artist_id=artist.id"),
